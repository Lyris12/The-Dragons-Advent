...
function ref.initial_effect(c)
	...
	e5:SetCountLimit(1)
	e5:SetCondition(ref.eqcon)
	e5:SetCost(ref.eqcost)
	e5:SetTarget(ref.eqotg)
	e5:SetOperation(ref.eqoop)
	c:RegisterEffect(e5)
	local e6=Effect.CreateEffect(c)
	e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e6:SetCode(EVENT_LEAVE_FIELD)
	e6:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e6:SetCondition(ref.damcon)
	e6:SetTarget(ref.damtg)
	e6:SetOperation(ref.damop)
	c:RegisterEffect(e6)
...
function ref.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return e:GetHandler():GetFlagEffect(id)==0 and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(aux.TRUE,tp,LOCATION_MZONE,0,1,c) end
...
function ref.eqotg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function ref.eqolimit(e,c)
	return e:GetOwner()==c
end
function ref.eqoop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler():GetEquipTarget()
	local tc=Duel.GetFirstTarget()
...
function ref.filter(c,e,tp,ec)
	return (c:GetPreviousEquipTarget()==ec and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP,1-tp)) or (c:GetEquipTarget()==ec and c:IsCanBeSpecialSummoned(e,0,tp,true,false,POS_FACEUP,1-tp))
end
function ref.damop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(1-tp,LOCATION_MZONE,1-tp)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,ref.filter,tp,0x3e,0x3c,1,1,nil,e,tp,e:GetLabelObject())
