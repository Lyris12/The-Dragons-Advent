...
function ref.filter2(c,e,tp,rk,rc,att)
	return c.dimensional_number_o<rk and (c:IsRace(rc) or c:IsAttribute(att))
		and c.spatial
...
function ref.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and ref.thfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(ref.thfilter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,ref.thfilter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,1,nil)
